...
/**
 * Creates a new assessment structure row (Category + Weight)
 */
...
// Initialize Student Course Cards
if (courseGrid) {
    courses.forEach(c => {
        ...
        card.onclick = () => {
            ...
            createStructureRow(1, structureList);
...
if (drawer) {
    ...
    document.getElementById("addMore").onclick = () => {
        const currentRows = structureList.querySelectorAll(".structure-row").length;
        createStructureRow(currentRows + 1, structureList);
    };
